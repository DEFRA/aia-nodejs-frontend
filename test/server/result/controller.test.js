...
describe('#resultController', () => {
  ...
  describe('API mode (mockData = false)', () => {
    ...
    async function buildHandler() {
      const mod = await import('../../../src/server/result/controller.js')
    ...
    describe('extractMarkdownContent edge cases via mocked fs', () => {
      afterEach(() => {
        vi.doUnmock('fs')
      })
      ...
      test('Should return markdownContent field value from mock data object', async () => {
        ...
        vi.doMock('fs', async () => {
          ...
          return {
            ...
            readFileSync: vi
              .fn()
              .mockReturnValue(
                '{"markdownContent": "# From markdownContent field"}'
              )
          }
        })
        ...
        expect(result.markdownContent).toBe('# From markdownContent field')
      ...
      test('Should return empty string when mock data has no recognised content fields', async () => {
        ...
        vi.doMock('fs', async () => {
          ...
          return {
            ...actual,
            readFileSync: vi.fn().mockReturnValue('{"unknownField": "value"}')
          }
        })
        ...
        // extractMarkdownContent returns '' — catch block assigns it directly (no fallback)
      ...
      test('Should handle parseJsonPayload catch path when inner string is not valid JSON', async () => {
        ...
        // readFileSync returns a JSON-encoded string (outer parse yields a string)
        // extractMarkdownContent receives "plain text", parseJsonPayload tries
        // JSON.parse("plain text") which throws → catch returns the string as-is
        vi.doMock('fs', async () => {
          const actual = await vi.importActual('fs')
          return {
            ...actual,
            readFileSync: vi
              .fn()
              .mockReturnValue('"plain text that is not json"')
          }
        })
        ...
        expect(result.markdownContent).toBe('plain text that is not json')
