...
describe('#policyDocumentsController', () => {
  ...
  test('Should redirect to list even when delete API throws a network error', async () => {
  ...
  test('Should render Add Policy Document form on GET /policy-documents/new', async () => {
    const { result, statusCode } = await server.inject({
      method: 'GET',
      url: '/policy-documents/new',
      headers: { cookie: authCookie }
    })

    expect(statusCode).toBe(200)
    expect(result).toContain('Add Policy Document')

    global.fetch = makeDefaultFetch()
  })

  test('Should show validation error when title is missing on new document submission', async () => {
    ...
    expect(statusCode).toBe(200)
    expect(result).toContain('There is a problem')
    expect(result).toContain('Enter a title or filename')

    global.fetch = makeDefaultFetch()
  })

  test('Should create a new policy document successfully on POST /policy-documents/new', async () => {
    global.fetch = vi.fn().mockImplementation((url, options) => {
      if (String(url).includes('/policy-documents/options')) {
        return Promise.resolve({
          ok: true,
      ...
      if (
        String(url).endsWith('/policy-documents') &&
        options?.method === 'POST'
      ) {
        return Promise.resolve({
          ok: true,
    ...
    const { result, statusCode } = await server.inject({
      ...
      headers: { cookie: authCookie },
      payload: {
        title: 'New Policy Doc',
        sourceUrl: 'https://example.com/new-policy',
        category: 'Security',
        type: 'SharePoint',
        isActive: 'true'
      }
    ...
    expect(statusCode).toBe(200)
    expect(result).toContain('Policy document created successfully')

    global.fetch = makeDefaultFetch()
  })

  test('Should show save failure message when API returns non-ok on new document', async () => {
    global.fetch = vi.fn().mockImplementation((url, options) => {
      if (String(url).includes('/policy-documents/options')) {
        return Promise.resolve({
          ok: true,
    ...
    expect(statusCode).toBe(200)
    expect(result).toContain('Failed to create document')

    global.fetch = makeDefaultFetch()
  })

  test('Should show error message when network fails on new document creation', async () => {
    global.fetch = vi.fn().mockImplementation((url, options) => {
      if (String(url).includes('/policy-documents/options')) {
        return Promise.resolve({
          ok: true,
    ...
    expect(statusCode).toBe(200)
    expect(result).toContain('An error occurred while creating the document')
