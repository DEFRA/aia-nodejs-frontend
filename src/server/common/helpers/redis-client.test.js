...
describe('#buildRedisClient', () => {
  ...
  describe('When only password is provided', () => {
    test('Should include password without requiring username', () => {
      buildRedisClient({
        ...config.get('redis'),
        useSingleInstanceCache: true,
        username: '',
        password: 'token-only-password'
      })

      expect(Redis).toHaveBeenCalledWith(
        expect.objectContaining({ password: 'token-only-password' })
      )
      expect(Redis).toHaveBeenCalledWith(
        expect.not.objectContaining({ username: expect.anything() })
      )
  ...
  describe('When a custom Redis port is configured', () => {
    test('Should use redisConfig.port for single instance connections', () => {
      buildRedisClient({
        ...config.get('redis'),
        useSingleInstanceCache: true,
        port: 6380
